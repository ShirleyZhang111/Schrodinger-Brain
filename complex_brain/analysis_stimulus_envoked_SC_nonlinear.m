...
for id_cortex = 1:node_items
    if(ismember(id_cortex,region) == 0)
        value_complex(id_cortex,1) = S0(id_cortex,time_complex(id_cortex)+1);
    end
end
...
figure;
plot(Amp_com(:,1),Amp_com(:,2),'LineWidth',0.7,'Color',[150  59  121]/255);hold on;
xlim([min(coup_strength),max(coup_strength)])

% Response time - Communicability
Time_com = [coup_strength,time_complex];
Time_com = sortrows(Time_com,1)/100;
figure;
plot(Time_com(:,1),Time_com(:,2),'LineWidth',1,'Color',[150  59  121]/255);hold on;
xlim([min(coup_strength),max(coup_strength)])