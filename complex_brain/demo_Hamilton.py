...
# Load preprocessed regional resting-state fMRI data 
dict_mat = loadmat(".\\Data\\TC_HCP_379.mat")
x = dict_mat["TC"]  # shape: (spatial, time)
x = stats.zscore(x, axis=1) # spatially z-scored
x = x[:,100:1000] # use 101~1000 time points (length = 900)
...
torch.save(model, '.\\Results\\HamiltonModel_Conv1d_900TRs.pt')
model.eval()


# Apply discrete Hilbert transform to fMRI signal 'x'
x = dict_mat["Data"]
...
# Save Learned Parameters (Auxilaray signal y and H)
