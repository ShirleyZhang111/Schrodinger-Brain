...
# learn auxilary signals and linear layer H from observed signals
class Hamilton_Model(nn.Module):
    def __init__(self, m):
        super(Hamilton_Model, self).__init__()
        self.m = m
        self.log_diag = nn.Parameter(torch.randn(m))
        self.off_diag = nn.Parameter(torch.randn(m, m)) 
    ...
    def forward(self, x):
        ...
        D = torch.diag(torch.exp(self.log_diag))
        L = torch.tril(self.off_diag, diagonal=-1) 
        H = D + 0.5*(L + L.T) 
