function [Q,G,lambda] = calc_coefficients_linear_group(inCfg,TC_group)
% Group-level linear model with orthogonal constraint
%     complex: min_{Q*Q = I} sum_s sum_j norm(Q reshape(z(:,j,s),[],1) - reshape(z(:,j+1,s),[],1))^2
%     real: min sum_s sum_j norm(Q reshape(z(:,j,s),[],1) - reshape(z(:,j+1,s),[],1))^2
% Input: 
%     z : space * time * subjects
%     T : time points for fitting
%     field : complex field or real field
% Output:
%     Q : the group-level transfer matrix
%     G : the eigenvectors of Q 
%     lambda : the eigenvalues of Q


% input configuration
% data_path = inCfg.data_path;
% save_path = inCfg.save_path;

if isfield(inCfg,'T');T = inCfg.T;else;T = [];end % Time points for fitting
if isfield(inCfg,'field');field = inCfg.field;else;field = 'complex';end 
if isfield(inCfg,'item_number');item_number = inCfg.item_number;else;item_number = 1;end % number of subjects
if isfield(inCfg,'idx');idx = inCfg.idx;else;idx = 1;end % space, number of node (region/voxel)

% items = dir(data_path);
% items = items(3:end);

W = zeros(idx,idx);
W1 = zeros(idx,idx); 
W2 = zeros(idx,idx);
if strcmp(field, 'complex') == 1
% Complex-valued linear model
    for k = 1:item_number
        % item = items(k).name;
        % data = load(strcat(data_path, '/', item));
        % TC = data.TC;
        TC = TC_group{1,k};
        % Complex-valued linear model
        hilbert_data = double(hilbert(zscore(TC'))).'; clearvars TC
        data = hilbert_data(:,T); clearvars hilbert_data    
        A = data(:,1:end-1);
        B = data(:,2:end);
        W = W + B*A';
    end    
    [U,~,V] = svd(W);
    Q = U*V';
    [G,D] = eig(Q);
    lambda = diag(D);
elseif strcmp(field, 'real') == 1
% Real-valued linear model
    for k = 1:item_number
        % item = items(k).name;
        % data = load(strcat(data_path, '/', item));
        % TC = data.TC;
        TC = TC_group{1,k};
        zscore_data = double(zscore(TC')).'; clearvars TC
        data = zscore_data(:,T); clearvars zscore_data    
        A = data(:,1:end-1);
        B = data(:,2:end);
        W1 = W1 + A*A'; W2 = W2 + B*A';
    end  
    Q = W2 / W1;
    [G,D] = eig(Q);
    lambda = diag(D);
end
    
% save([save_path 'Group_rfMRI_resting_Q_G_lambda_' field '.mat'], 'Q','G','lambda');
