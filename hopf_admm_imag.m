function [x,Z,res,flag,flag0] = hopf_admm_imag(A,B,C,mu,maxit,tol)
%HOPF_ADMM solves
%   min sum_j 0.5*norm(A(:,:,j)*x + 1i*Z*B(:,j) - C(:,j))^2 + mu*norm(Z,ell_1)
%Input:
...
for iter = 1:maxit
    ...
    % update Y
    W = reshape(A*x,[n,t]);
    Y = (Y.*tau + Z.*rho - lambda + real(W-C)*imag(B)' - imag(W-C)*real(B)')/dF;
